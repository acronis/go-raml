...
func (r *RAML) decodeLibrary(f io.Reader, path string) (*Library, error) {
	...
	for pair := lib.Uses.Oldest(); pair != nil; pair = pair.Next() {
		...
		sublib, err := r.parseLibrary(filepath.Join(baseDir, include.Value))
		if err != nil {
			se := stacktrace.NewWrapped("parse uses library", err, path, stacktrace.WithType(stacktrace.TypeParsing), stacktrace.WithPosition(&include.Position))
			if st == nil {
				st = se
			} else {
				st = st.Append(se)
...
func (r *RAML) parseFragment(f io.ReadSeeker, fragmentPath string, pOpts *parserOptions) error {
	...
	err = r.resolveShapes()
	if err != nil {
		st = stacktrace.NewWrapped("resolve shapes", err, fragmentPath, stacktrace.WithType(stacktrace.TypeParsing))
	}
	err = r.resolveDomainExtensions()
	if err != nil {
		se := stacktrace.NewWrapped("resolve domain extensions", err, fragmentPath, stacktrace.WithType(stacktrace.TypeParsing))
		if st == nil {
			st = se
		} else {
			st = st.Append(se)
	...
	if pOpts.withUnwrapOpt {
		err = r.UnwrapShapes()
		if err != nil {
			se := stacktrace.NewWrapped("unwrap shapes", err, fragmentPath, stacktrace.WithType(stacktrace.TypeParsing))
			if st == nil {
				st = se
			} else {
				st = st.Append(se)
	...
	if pOpts.withValidateOpt {
		...
		if err != nil {
			se := stacktrace.NewWrapped("validate shapes", err, fragmentPath, stacktrace.WithType(stacktrace.TypeParsing))
			if st == nil {
				st = se
			} else {
				st = st.Append(se)
